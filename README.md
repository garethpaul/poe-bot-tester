...
- Run `make root-test` for hostile Make root and npm command regression
  coverage without contacting Poe.
...
- A terminal chunk stream failure releases its exact in-memory session before
  error emission so a stale bot binding cannot block later reuse.
...
- Chunk sessions enforce an exact chunk sequence: new sessions start at chunk 0,
  and replayed or skipped chunks receive HTTP 409 without advancing state.
- The test-file fixture API rejects unknown test file types, including inherited
  object keys, before decoding fixture data.
- Full analysis resolves bundled test-file fixtures against the incoming
  request's same origin, so deployed checks do not depend on a development-only
  localhost port; inherited URL credentials are removed before the internal fetch.
